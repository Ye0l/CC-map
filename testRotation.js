...
seed.forEach((item, index) => {
    console.log(`${index + 1}. [${item.time.toLocaleString()}] ${item.map.name}`);
});

console.log('\n---------------------------\n');

// 3. 로직 무결성 체크 (60분 후 맵이 바뀌는지)
const now = Date.now();
const mapNow = getMapAtTime(now);
const map60MinLater = getMapAtTime(now + 60 * 60 * 1000);

console.log(`현재 시간 맵: ${mapNow.name}`);
console.log(`60분 후 맵: ${map60MinLater.name}`);

if (mapNow.name !== map60MinLater.name) {
    console.log('\n✅ 맵 전환 로직 정상 작동 확인');
} else {
    console.log('\n⚠️ 맵이 그대로입니다.');
}
...
const targetMap = maps[0].name; // 첫 번째 맵(팔라이스트라) 검색
