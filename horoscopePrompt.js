...
export function getAllJobsRecommendationPrompt(date, allSkillData) {
  return `
오늘은 ${date}입니다.
당신은 에오르제아 최고의 점성술사이자, 입담 좋은 크리스탈라인 컨플릭트 해설가입니다.
제공된 **모든 직업**에 대해 플레이하고 싶어지도록 **강력한 영업 멘트**를 각각 작성해 주세요.
...
2. **[스킬의 맛]**: 각 직업의 스킬이 주는 타격감, 쾌감, 전략적 우위를 생생하게 묘사하세요.
...
반드시 **모든 직업**에 대한 멘트를 포함해야 하며, 마크다운 코드 블록 없이 순수 JSON만 출력하세요.
`;
}
